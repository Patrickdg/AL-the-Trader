---
title: "Regression Modelling, Algorithmic Trading"
...
y_col <- c('next_close')
remove_y_cols <- c('next_close_2', 'next_close_3','next_close_5','next_close_10')
...
nearzeros <- nearZeroVar(train); nearzeros <- names(train)[nearzeros]
train <- train[setdiff(names(train), nearzeros)]
test <- test[setdiff(names(test), nearzeros)]
...
set.seed(1111)
train <- train[, -which(names(train) == "next_close")]
test <- test[, -which(names(test) == "next_close")]
